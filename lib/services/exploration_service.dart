...
class ExplorationService {
  /// Returns a uid-scoped key so each account's exploration path is separate.
  /// Falls back to the legacy key if no user is signed in (shouldn't happen).
  static String get _storageKey {
    final uid = FirebaseAuth.instance.currentUser?.uid;
    return uid != null ? 'visited_points_$uid' : 'visited_points';
  }
  ...
  /// Clears the exploration path for the currently signed-in user.
  ...
  // Returns true if newPoint is far enough from all existing points
